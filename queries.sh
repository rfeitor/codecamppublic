...
echo "$($PSQL "SELECT distinct(name) FROM games INNER JOIN teams ON (winner_id = team_id or opponent_id = team_id) where year = 2014 and round = 'Eighth-Final' order by name")"
